...
//                "<p><a href='http://sisob.lcc.uma.es/' target='_blank'><img src='logos/logo-SISOB-wb.svg' style='width: 28em' /></a></p>" +
                "<p style='padding-top: 15px; padding-bottom: 10px; margin-left: auto; margin-right: auto'>This is the LAK 2013 special version of a network analytics<br />workbench developed together with the <a href='http://sisob.lcc.uma.es/' target='_blank'>SISOB project</a>.</p>" +
...
(function () {
    ...
    lang.extend(WireIt.WiringEditor, WireIt.BaseEditor, {
        ...
        sortModules: function(modules){
            ...
            var result = [];
            var toHandle = modules;

            //for all order-layers
            for (var i = 0; i < FILTERORDER.length; i++){
                var tmp = [];
                //find all modules with same category
                for (var j = 0; j < toHandle.length; j++){
                    if (toHandle[j].category.toString() == FILTERORDER[i]){
                        tmp.push(toHandle[j]);
                    }
                }
                ...
                for(var h = 0; h < sorted.length; h++){
                    result.push(sorted[h]);
                }

            }
            //make sure all filters will be returned
            if(result.length < modules.length){
                //some categories werent defined, search: which ones..
                for (var c = 0; c < modules.length; c++){
                    var isThere = false;
                    for (var b = 0; b < result.length; b++){
                        if (result[b].name == modules[c].name){
                            isThere = true;
                        }
                    }
                    if(!isThere){
                        result.push(modules[c]);
                    }
                }
            }
            return result;
                ...
                this.adapter.getLoadableWirings(function (wirings) {
                    ...
                    for (var i = 0; i < wirings.length; i++) {
                        ...
                        if (wiring.name != "autosave"){
                            ...
                            tableOWN +=    "<tr " + rowStyle + "onClick=\"javascript:highlightLoadSelection(this)\" loadid='" + (counter+1) + "' name='loadSelection'>"
                    ...
                    for (var i = 0; i < wirings.length; i++) {
                        ...
                        if (wiring.name == "autosave"){
                            ...
                            tableAUTO +=    "<tr " + rowStyle + "onClick=\"javascript:highlightLoadSelection(this)\" loadid='" + (counter+1) + "' name='loadSelection'>" +
            ...
            if (!editor.loadResultsDialogOpen) {
                ...
                this.adapter.getLoadableResults(function (results) {
                    ...
                    for (var i = 0; i < results.length; i++) {
                        ...
                        table +=    "<tr " + rowStyle + " onClick=\"javascript:highlightLoadSelection(this)\" loadid='" + (i+1) + "' name='loadSelection'>" +
